...
<table class="easyui-datagrid" id="itemParamList" title="商品列表" 
       data-options="singleSelect:false,collapsible:true,pagination:true
       	   ,url:'/item/param/list',method:'get',pageSize:30,toolbar:itemParamListToolbar">
    <thead>
        <tr>
        	<th data-options="field:'ck',checkbox:true"></th>
        	<th data-options="field:'id',width:60,align:'center'">ID</th>
        	<th data-options="field:'itemCatId',width:80,align:'center'">商品类目ID</th>
        	<th data-options="field:'itemCatName',width:100,align:'center'">商品类目</th> 
            <th data-options="field:'paramData',width:300,formatter:formatItemParamData">规格(只显示分组名称)</th> 
            <th data-options="field:'created',width:130,align:'center',formatter:JINGXI.formatDateTime">创建日期</th>
            <th data-options="field:'updated',width:130,align:'center',formatter:JINGXI.formatDateTime">更新日期</th>
        </tr>
    </thead>
</table>
<div id="itemParamEditWindow" class="easyui-window" title="编辑商品" 
	data-options="modal:true,closed:true,iconCls:'icon-save',href:'/item-param-edit'" 
	style="width:80%;height:80%;padding:10px;">
</div>
<script>

	var paramData = [] ;//保存paramData

	function formatItemParamData(value , index){
		paramData.push(value);
		var json = JSON.parse(value);//解析json
		var array = [];
		$.each(json,function(i,e){//i 当前的位置，e  当前的元素
			array.push(e.group);
		});
		return array.join(",");//元素通过 指定的分隔符 进行分隔
	}
        ...
        handler:function(){
        	...
        	$("#itemParamEditWindow").window({
        		onLoad :function(){
        			...
        			data.created=JINGXI.formatDateTime(data.created,0);
        			data.updated=JINGXI.formatDateTime(data.updated,0);
        			$("#itemParamEditForm").form("load",data);
        			//获取完整参数的值
        			JINGXI.init({
        				"cid" : data.cid,
        				fun:function(node){
        					JINGXI.changeItemParam(node, "itemeEditForm");
        				}
        			});
        		}
        	}).window("open");
