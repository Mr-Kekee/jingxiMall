...
<div class="hide param_div">
	<ul class="param_div_ul">
		<li class="groups_name"><label></label></li>
		<li >
			<ul>
				<li class="params_name">
					<span>|----</span>
					<label></label>
					<input name="param_input" style="width:98px;">
				</li>
				<!-- 接下一个参数项 -->
			</ul>
		</li>
...
<script type="text/javascript">
	...
	$(function(){
		//创建富文本编辑器
		itemAddEditor = JINGXI.createEditor("#itemAddForm [name=desc]");
		//初始化类目选择和图片上传器
		JINGXI.init({fun:function(node){
			//根据商品的分类id取商品 的规格模板，生成规格信息。
			//JINGXI.changeItemParam(node, "itemAddForm");
		}});
	});
	...
	function submitForm(){
		...
		var paramJson = [];
		$("#itemAddForm .param_div_ul").each(function(i,e){
			var group = $(e).find("li").eq(0).find("label").text();
			var param_lis = $(e).find("li").eq(1).find("li");
			var ps = [];
			for(var i=0;i<param_lis.length;i++){
				var param_li = param_lis.eq(i);
				var param_name = param_li.find("label").text();
				var param_val = param_li.find("input").val();
				//加判断再添加
				if(param_val!=""){
					ps.push({
						"k" : $.trim(param_name),
